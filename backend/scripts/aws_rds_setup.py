...
def test_connection(host: str, port: str, user: str, password: str, database: str) -> bool:
...
def import_to_rds(backup_file: str, host: str, port: str, user: str, password: str, database: str) -> bool:
...
DB_NAME=literattus
    ...
    table.add_row("Password", "*" * len(rds_password))
    ...
    if not test_connection(rds_host, rds_port, rds_user, rds_password, "mysql"):
    ...
    if not create_database(rds_host, rds_port, rds_user, rds_password, rds_database):
        return
    ...
    if Confirm.ask("\\n[bold]Do you want to migrate data from your local database?[/bold]"):
        ...
        if backup_file and Confirm.ask("\\n[bold]Import this backup to AWS RDS?[/bold]"):
            import_to_rds(backup_file, rds_host, rds_port, rds_user, rds_password, rds_database)
